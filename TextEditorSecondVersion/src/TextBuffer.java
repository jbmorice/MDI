public class TextBuffer {
	...
	public void delete(int begin, int end) {
		String newContent;
		if (begin < end) {
			newContent = this.content.substring(0, begin + 1) + this.content.substring(end, this.content.length() - 1);
		} else {
	...
	public void insert(Integer position, String content) {
		String newContent = this.getContentWithoutCaret().substring(0, position) + content + CARET
	...
	public void setCaretPosition(int newPosition) {
		...
		String newContent = this.getContentWithoutCaret().substring(0, newPosition) + CARET
	...
	public TextBufferMemento save() {
		return new TextBufferMemento(this.content, this.caretPosition, this.startSelectionPosition,
				this.endSelectionPosition, this.clipboard.getContent());
	}
	
	public void restore(TextBufferMemento memento) {
		this.content = memento.getContent();
		this.caretPosition = memento.getCaretPosition();
		this.startSelectionPosition = memento.getStartSelectionPosition();
		this.endSelectionPosition = memento.getEndSelectionPosition();
		this.clipboard.setContent(memento.getClipboardContent());
